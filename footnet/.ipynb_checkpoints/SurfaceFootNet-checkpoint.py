...
class SurfaceFootDataset(Dataset):
    """
    Dataset class for SurfaceFootNet model.
    """
    def __init__(self, data, input_met):
        """
        Initialize the dataset with data and input meteorology.
        data: list of tuples (timestamp, rlon, rlat)
        input_met: instance of SurfaceMeteorology class
        1. timestamp: datetime object
        2. rlon: receptor longitude
        3. rlat: receptor latitude
        4. input_met: instance of SurfaceMeteorology class
        """
        self.data = data
    ...
    def get_distance(self, rlon, rlat, lat, lon):
        """
        Calculate the great-circle distance between a receptor point and all grid points.
        rlon: receptor longitude
        rlat: receptor latitude
        lat: 2D array of latitudes
        lon: 2D array of longitudes
        Returns distance array in kilometers.
        """
...
class SurfaceFootNet():
    """
    Class to handle SurfaceFootNet model loading and inference.
    """
    def __init__(self, model_path):
        """
        Initialize the SurfaceFootNet model.
    ...
    def post_processing(self, pred):
        ...
        pred[np.where(pred<5e-6)] = 0
        return pred
    
    def run_inference(self, receptors, input_met):
        """
        Run inference on the input data.
        receptors: list of tuples (timestamp, rlon, rlat)
        input_met: instance of SurfaceMeteorology class
        Returns:
        foots: numpy array of footprints corresponding to receptors
        reference_indices: numpy array of receptor indices corresponding to footprints
        reference_timestamps: list of timestamps corresponding to footprints
        reference_rlons: numpy array of receptor longitudes corresponding to footprints
        reference_rlats: numpy array of receptor latitudes corresponding to footprints
        """
        batch_DG = DataLoader(SurfaceFootDataset(receptors, input_met),  batch_size=8, shuffle=False, num_workers=8, pin_memory=True)
        self.model.eval()
        with torch.no_grad():
