...
simulation_step <- function(X, rm_dat = T, stilt_wd = getwd(), lib.loc = NULL,
                            ...
                            run_trajec = T, run_foot = T, overwrite = F,
                            shuffle = F, siguverr = NA, sigzierr = NA,
                            ...
                            yres = xres, nx = 100, ny = 100, zicontroltf = 0, ziscale = 0,
                            z_top = 25000, zcoruverr = NA) {
  #if (shuffle) X <- sample(X) # Randomize the order to balance the load?
  try({
    ...
    if (run_trajec) {
      ...
      # Read an old particle file or run a new one?
      if ( (!overwrite) && 
      ...
      if ( (overwrite) ||
           (!file.exists(output$file)) ||
           (!file.exists(output_wd, 'particles',basename(output$file))) ) {
    ...
    if ( (hnf_plume) && (run_foot) )
      try( particle <- calc_plume_dilution(particle, numpar, r_zagl, veght) )
