...
calc_footprint <- function(p, output = NULL, r_run_time,
                           ...
                           xmn, xmx, xres, ymn, ymx, yres = xres, nx, ny) {
  ...
  # <- AJT
  max_glong <- linspace(xmn - (xbuf*xres), xmx + ((xbuf - 1)*xres), nx+xbuf*2-1)
  max_glati <- linspace(ymn - (ybuf*yres), ymx + ((ybuf - 1)*yres), ny+ybuf*2-1)
  ...
  size <- dim(foot)
max_glong[(xbuf+1):(size[1]-xbuf+1)]
max_glati[(ybuf+1):(size[2]-ybuf+1)]
