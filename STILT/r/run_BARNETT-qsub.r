...
qsub_config <- override_qsub_config(
    name = "BARNETT_QSUB", # this name will show up in qstat
    num_cores = 32, # the number of cores to allocate per element in X
    memory = "10G", # memory per core per task
    max_wall_time = "24:00:00", # allow each task to run for 24h
    verbose=TRUE,
    max_running_tasks=96 #If you have long jobs and there are many other users on the cluster, it is recommended you set this value to a reasonable number, such as 1/4th the total number of nodes * number of cores per node.
  )
...
fjul   <- ncvar_get(ncfile, varid="fjul")
...
tID <- as.POSIXct('2019-03-01 00:00:00', tz = 'PST') <= run_times & run_times < as.POSIXct('2019-03-05 00:00:00', tz = 'PST')
...
xlims <- c(-125.0, -120.0) # Larger Bay Area domain
ylims <- c(  36.0,   40.0) # Larger Bay Area domain
# Make the grid
...
if ((nchar(paste0(met_directory, met_file_format)) + 2) > 80) {
  ...
  met_loc <- file.path(path.expand('~/met_symbolic'))
...
output <- stilt_apply(X = 1:nrow(receptors), FUN = simulation_step,
                      qsub = qsub, slurm_options = slurm_options,
