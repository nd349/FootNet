...
def transform_func_12h(_xx, _6xx, _12xx, _yy, comb_plume, gp_first, transform=''):
    ...
    # _xx[:, :, 0] = zstandard(_xx[:, :, 0])   Do this for Gaussian Plume
...
def transform_func_18h(_xx, _6xx, _12xx, _18xx, _yy, comb_plume, gp_first, transform=''):
    '''
    xx: (400, 400, 14)
    yy: (400, 400)
    predlist: 'GPR', 'U10M', 'V10M', 'PBLH', 'PRSS', 'SHGT', 'T02M', 'ADS',
    ...
    gp_first = zstandard(gp_first)
    gp_first = gp_first[:, :, np.newaxis]
    return np.concatenate([gp_first, _xx, _6xx, _12xx, _18xx, comb_plume, _yy], axis=-1), original_yy
...
class FootDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        _pred = data['_pred'][int(np.array(data['clat_shift_index'])[0])-200:int(np.array(data['clat_shift_index'])[0])+200, int(np.array(data['clon_shift_index'])[0])-200:int(np.array(data['clon_shift_index'])[0])+200]
        _6hpred = data['_6hpred'][int(np.array(data['clat_shift_index'])[0])-200:int(np.array(data['clat_shift_index'])[0])+200, int(np.array(data['clon_shift_index'])[0])-200:int(np.array(data['clon_shift_index'])[0])+200]
        _12hpred = data['_12hpred'][int(np.array(data['clat_shift_index'])[0])-200:int(np.array(data['clat_shift_index'])[0])+200, int(np.array(data['clon_shift_index'])[0])-200:int(np.array(data['clon_shift_index'])[0])+200]
        _18hpred = data['_18hpred'][int(np.array(data['clat_shift_index'])[0])-200:int(np.array(data['clat_shift_index'])[0])+200, int(np.array(data['clon_shift_index'])[0])-200:int(np.array(data['clon_shift_index'])[0])+200]
        _24hpred = data['_24hpred'][int(np.array(data['clat_shift_index'])[0])-200:int(np.array(data['clat_shift_index'])[0])+200, int(np.array(data['clon_shift_index'])[0])-200:int(np.array(data['clon_shift_index'])[0])+200]
        ...
        elif self.backhours == 18:
            tempxy, original_yy = transform_func_18h(_pred, _6hpred, _12hpred, _18hpred, label, combined_gp, gp_first, transform=self.transform)
...
class Trainer():
    def __init__(
        ...
        if os.path.exists(self.snapshot_path):
            print(f"[GPU: {self.global_rank}] Loading snapshot: {self.snapshot_path}")
    ...
    def _valid_epoch(self, epoch):
        ...
        return running_loss, running_R2, running_foot_sums, running_foot_pred_sums

    def train(self, max_epoch:int):
        train_b_sz = len(next(iter(self.train_DG))[0])
        valid_b_sz = len(next(iter(self.valid_DG))[0])
        print(f"[GPU: {self.global_rank}] Training begins ....{self.args.world_size}")
        for epoch in range(self.epochs_run, max_epoch):
            ...
            print(f"[GPU{self.global_rank}] Epoch {epoch} | Batchsize: {train_b_sz} | Steps: {len(self.train_DG)} | Training loss: {round(train_loss.item(), 3)} | Training R2: {round(train_R2.item(), 3)} | Foot sum: {round(train_foot_sum.item(), 3)} | Pred sum: {round(train_pred_sum.item(), 3)} | Time taken: {round((time.time()-start)/60, 3)} minutes")
            ...
            if self.valid_DG:
                ...
                valid_loss, valid_R2, valid_foot_sum, valid_foot_pred_sum = self._valid_epoch(epoch)
                ...
                print(f"[GPU{self.global_rank}] Epoch {epoch} | Batchsize: {valid_b_sz} | Steps: {len(self.valid_DG)} | Validation loss: {round(valid_loss.item(), 3)} | Validation R2: {round(valid_R2.item(), 3)} | Foot sum: {round(valid_foot_sum.item(), 3)} | Pred sum: {round(valid_foot_pred_sum.item(), 3)} | Time taken: {round((time.time()-start)/60, 3)} minutes")
...
def get_dataloaders():
    ...
    training_file = "training_files/training_500k_CONUS.csv"
    validation_file = "training_files/validation_400k_CONUS.csv"
    test_file = "training_files/test_400k_CONUS.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--transform', default='log-epsilon', type=str, help='Transformation for the footprints')
